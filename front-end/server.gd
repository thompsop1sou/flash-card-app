...
func request_open(flashcard_set_name: String, callback: Callable = func(_rslt, _code, _hdrs, _body): pass) -> void:
	...
	print(flashcard_set_data)
	# Actually make the request
	http_request.request(full_url, [server_key], HTTPClient.METHOD_POST, flashcard_set_data)
...
func request_save(flashcard_set_name: String, flashcard_set_text: String, callback: Callable = func(_rslt, _code, _hdrs, _body): pass) -> void:
	# Set up a new http_request
	var http_request := HTTPRequest.new()
	add_child(http_request)
	http_request.request_completed.connect(func(_rslt, _code, _hdrs, _body): http_request.queue_free())
	...
	var server_key: String = "FLASHCARD_SERVER_KEY:" + OS.get_environment("FLASHCARD_SERVER_KEY")
	var flashcard_set_data: String = "{\"name\": \"" + flashcard_set_name + "\", \"cards\": " + flashcard_set_text + " }"
	print(flashcard_set_data)
	# Actually make the request
	http_request.request(full_url, [server_key], HTTPClient.METHOD_POST, flashcard_set_data)
	http_request.request_completed.connect(callback)
